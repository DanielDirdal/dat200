...
# Kjøretid: O(V + E)
def dfs_rekursiv(graf, node, tidsstempel, liste, noder_proesessert, noder_gjenstaaende):
    ...
    graf.set_starttidspunkt(node, tidsstempel)
    naboer = graf.get_naboer(node)              # Theta(antall naboer)
    for nabo in naboer:
        if graf.get_kostnad(nabo) is None:
            graf.set_kant_type(node, nabo, "Tree-edge")
            dfs_rekursiv(graf, nabo, tidsstempel, liste, noder_proesessert, noder_gjenstaaende)
    ...
    noder_proesessert.add(node)                 # Theta(1) add på hashet
