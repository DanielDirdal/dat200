class Binaertre:
    ...
    def er_bladnode(self):
        if self.venstre_barn is None and self.__hoyre_barn is None:
            return True
        return False
    ...
    def rekursiv_postorder_utregning(self):
        ...
        hoyre_verdi = self.__hoyre_barn.rekursiv_postorder_utregning()
        if self.data == "+":
    ...
    def rekursiv_bladnode_teller(self, teller=0):
        if self.er_bladnode():
            teller += 1
        else:
            if self.venstre_barn is not None and self.__hoyre_barn is not None:
                return self.venstre_barn.rekursiv_bladnode_teller(teller=teller) + self.hoyre_barn.rekursiv_bladnode_teller(teller=teller)
            if self.venstre_barn is not None:
                return self.venstre_barn.rekursiv_bladnode_teller(teller=teller)
            if self.__hoyre_barn is not None:
                return self.hoyre_barn.rekursiv_bladnode_teller(teller=teller)
        return teller
...
class BinaertrePreorderIterator:
    ...
    def __next__(self):
        if self.treet is None:
            return "Ingen noder"
        while len(self.stabel) > 0:
            element = self.stabel.pop()
            print(element.data)
            if element.hoyre_barn is not None:
                self.stabel.append(element.hoyre_barn)
            if element.venstre_barn is not None:
...
if __name__ == "__main__":
    ...
    print("Preorder rekursiv")
    rot.rekursiv_preorder_utskrift()
    print("Posterorder rekursiv")
    #print(rot.rekursiv_postorder_utregning())
    print("Inorder")
    #rot.rekursiv_inorder_formelutskrift()
    print()
    print("Iterativ preorder: ")
    for element in rot:
        print(element)
    print()
    print(f"Antall bladnoder: {rot.rekursiv_bladnode_teller()}")