from trestruktur import student as std
...
class Node:
    ...
    def rekursiv_preorder_utskrift(self, nivaa=0):
        for i in range(nivaa):
            print("\t", end="")
        print(f"({self.nokkel}, {self.verdi}) - høyde: {self.hoyde} og størresle: {self.storrelse}")
...
class AVLTre:
    ...
    def skriv_ut_rekursjon_inorder(self):
        variabel = self.inorderTraversal(self.rot)
        return variabel

    def inorderTraversal(self, root):
        res = []
        if root:
            res = self.inorderTraversal(root.venstre_barn)
            res.append(root.nokkel)
            res = res + self.inorderTraversal(root.hoyre_barn)
        return res
    ...
    def vedlikehold_hoydeinfo(self, node):
        nv_node = node
        while nv_node is not None:
            ...
            self.vedlikehold_storrelse(nv_node)
            nv_node = nv_node.forelder

    def vedlikehold_storrelse(self, node):
        nv_node = node
        if nv_node is None:
            return
        if nv_node.er_bladnode():
            nv_node.storrelse = 1
        elif nv_node.forelder is not None:
            if nv_node.venstre_barn is not None and nv_node.hoyre_barn is not None:
                nv_node.storrelse = nv_node.venstre_barn.storrelse + nv_node.hoyre_barn.storrelse + 1
            if nv_node.venstre_barn is not None and nv_node.hoyre_barn is None:
                nv_node.storrelse = nv_node.venstre_barn.storrelse + 1
            if nv_node.venstre_barn is None and nv_node.hoyre_barn is not None:
                nv_node.storrelse = nv_node.hoyre_barn.storrelse + 1
        else:
            if nv_node.venstre_barn is not None and nv_node.hoyre_barn is not None:
                nv_node.storrelse = nv_node.venstre_barn.storrelse + nv_node.hoyre_barn.storrelse + 1
            if nv_node.venstre_barn is not None and nv_node.hoyre_barn is None:
    ...
    def enkel_hoyre_rotasjon(self, nv_node):
        # Flytter noden sitt høyre barn opp
        if nv_node.forelder is None:
    ...
    def storrelse(self, node):
        if node is None:
            return 0
        return node.hoyde

    def finn_k_ende_time(self, k):
        nv_node = self.rot
        variabel = self.finn_k_ende_time_rekursjon(k, nv_node)
        return variabel

    def finn_k_ende_time_rekursjon(self, k, node):
        nv_node = node
        if nv_node.venstre_barn is not None and nv_node.venstre_barn.storrelse == k - 1:
            print(nv_node.nokkel)
            return nv_node.nokkel
        elif nv_node.venstre_barn is not None and nv_node.venstre_barn.storrelse > k - 1:
            nv_node = nv_node.venstre_barn
            self.finn_k_ende_time_rekursjon(k, nv_node)
        elif nv_node.venstre_barn is not None and nv_node.venstre_barn.storrelse < k - 1:
            ny_k = k - nv_node.venstre_barn.storrelse - 1
            nv_node = nv_node.hoyre_barn
            self.finn_k_ende_time_rekursjon(ny_k, nv_node)
        elif nv_node is not None and nv_node.er_bladnode():
            print(nv_node.nokkel)
            return nv_node.nokkel
        elif nv_node.venstre_barn is None and nv_node.hoyre_barn is not None:
            if k == 1:
                print(nv_node.nokkel)
            else:
                print(nv_node.hoyre_barn.nokkel)
    ...
    def get(self, nokkel):
        ...
        if noden is None:
            print(f"Finner ikke nøkkelen {nokkel}")
            return 0
        return noden.verdi
    ...
    def delete(self, nokkel):
        ...
        if noden.er_bladnode():
            ...
            self.vedlikehold_storrelse(noden.forelder)
            noden.forelder = None
...
if __name__ == "__main__":
    ...
    print()
    map.skriv_treemap()
    print("Finn k ende element")
    #print(map.finn_k_ende(11))
    #print("Finn k ende element time")
    #map.finn_k_ende_time(11)
    for i in range(1, 16):
        print(f"{map.finn_k_ende(i)} skal være lik: ", end='')
        map.finn_k_ende_time(i)
    print()
    print(map.first())
    print(map.between(5, 22))
    print()
    map.skriv_treemap()
    map.delete(34)
    map.delete(20)
