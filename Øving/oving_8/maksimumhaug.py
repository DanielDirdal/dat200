class Prioritetsko:
    ...
    # Kjøretid: Theta(1)
    def er_storre(self, index, prioritet):
    ...
    def boble_ned(self, index):
        while (self.er_storre(self.venstre_barn(index), self.liste[index][1]) or self.er_storre(self.hoyre_barn(index), self.liste[index][1])):
    ...
    def add(self, verdi, prioritet):
        ...
        self.liste.append(verdi_tuppel)                 # Theta(1) amortized
        self.elementfinner[verdi] = len(self.liste) - 1
        self.boble_opp(len(self.liste) - 1)             # O(1) best case, O(log(n) worst case

    # Tar ut og returnere verdien med høyest verdi i "prioritet" variabelen
    ...
    def remove(self):
        ...
        del self.liste[-1]                              # Å slette det siste element i en array-liste Theta(1)
        if len(self.liste) > 1:
            self.boble_ned(1)
        del self.elementfinner[returverdi]
        return returverdi

    # Finner verdien med laveste verdi i "prioritet" variabelen og returnere den, men fjerne den ikke
    # Kjøretid Theta(1)
    def peek(self):
        if len(self) < 1:
            raise ValueError("Har ingen elementer!")
