...
def haandter_ankomst(passasjerer, hendelsekoe, systemtid):
    print(f"Passasjer ankommer på tid: {systemtid}. Passajserkølengde {len(passasjerer)}")
    passasjerer.append(systemtid)
    ny_tid = systemtid + random.randint(5, 90)
    hendelsen = Hendelse(HENDELSE_ANKOMMET, ny_tid)
    hendelsekoe.add(hendelsen, ny_tid)
...
def haandter_behandlet(passasjerer, hendelseskoe, systemtid):
    if len(passasjerer) == 0:
        ...
        ny_tid = systemtid + 300
    else:
        print(f"Behandlet passasjer på tid: {systemtid}. Passajserkølengde {len(passasjerer)}")
        passasjerer_ankomsttid = passasjerer.popleft()
        ventetid = systemtid - passasjerer_ankomsttid
        ny_tid = systemtid + 60
...
def kjor_simulering():
    ...
    print(f"Antall ganger kontroll er ledig: {antall_ledige_ganger}")
